...
проверкой хоста, отличиями server `.env`, созданием сотрудника, retention,
порядком миграций с остановленными writers и границами отката без backup.
В [VPS runbook](runbooks/vps-caddy.md) исправлено применение переменной
доверенного peer через пересоздание edge и добавлена диагностика FRP `502`.
Изменения только в документации; деплой не выполнялся.
