...
Для подключения нового киоска: «Экраны → Добавить экран», введите название,
выберите площадку и нажмите «Создать экран». В появившейся карточке скопируйте
token в настройки этого киоска. Для второго киоска той же площадки создайте
ещё один экран; используйте его отдельный token. Настройки поиска и реклама
принадлежат площадке. Создание площадки само по себе экран не создаёт.

Последующее дополнение оператора выполнено: на `/staff/spas` есть кнопка
«Добавить площадку», название и список GMT+1…GMT+10 (по умолчанию GMT+7).
Новые значения — фиксированные смещения без сезонного перевода времени;
существующие часовые пояса площадок сохраняются.
Создание назначает общую revision и независимые начальные настройки: сегодня,
similarity .38, quality .5, детекторы .9/.5. Доступ operator/developer, CSRF.
Backend перезапущен, Caddy перечитан; форма и отклонение неверного POST проверены
через локальный HTTPS без создания площадки в рабочей БД.
...
только общую активную revision; UI создания описан выше.
