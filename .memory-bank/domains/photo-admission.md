...
page includes `candidates/` objects and deletes keys absent from
`Photo.original_object_key`; other known namespaces are specified in the
[private object cleanup contract](../contracts/photo-inventory-api.md#private-object-cleanup).
It holds a PostgreSQL admission lock across the scan, so no concurrent Photo
upload can publish or stage a candidate during deletion;
