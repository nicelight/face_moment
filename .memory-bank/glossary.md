...
| Photo visibility / soft-deleted Photo | Один признак активности Photo. Soft delete сохраняет Photo, media, face data и pipeline state, исключает Photo из новых participant search/results и recent counters, но не ломает уже выданную session; restore возвращает сохранённое состояние и timestamps. | Hard purge, повторным upload/processing или отдельным Photo pipeline state. |
| Global hard-purge run | Одна resumable project-wide операция над fixed snapshot всех soft-deleted Photos на момент подтверждения, с wait за shared worker и completed/total progress. Restore snapshot members запрещён до completion. Run удаляет Photo-owned data, сохраняя Promo sessions, core Attempts и diagnostic evidence; клиенты пропускают отсутствующую media. | Per-photo `purge_pending`, общей jobs-системой, удалением session/Attempt/evidence или динамическим изменением текущего snapshot. |
...
| Serving pipeline | Одна выбранная pipeline revision, которая обслуживает participant-facing поиск данной СПА. | Benchmark-сравнением двух pipelines или multi-model ensemble. |
...
| Attempt | Одна server-admitted автоматическая capture/search/display execution со stage timestamps и outcome, включая unsuccessful outcome. Client-only offline trigger может быть доставлен best-effort, но не обязан создать durable Attempt. Игнорируемый sensor event во время занятого состояния не создаёт attempt. | Только successful Promo, QR session или любым локальным sensor trigger. |
