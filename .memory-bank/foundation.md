...
- Smoke path: future fake-FaceEngine substrate path through HTTPS application, PostgreSQL/pgvector and private MinIO.
- Test command: not_available - Foundation must establish one project-native test command.
- Evidence: successful commands plus PostgreSQL and MinIO read/write/delete, realtime warmup seam, SpaPromoClient build and one substrate end-to-end smoke.

## Feature Pressure Map

| Feature | Pressure | Foundation Response | Probe | Status |
|---|---|---|---|---|
| FT-001, FT-002, FT-012 | Shared PostgreSQL/MinIO baseline, migration/init and backend/worker entrypoints. | Compose storage, one schema path and fake processing seam. | Storage roundtrip and durable row visibility across restart. | planned |
...
|---|---|---|
| Photo/Attempt/purge schemas and endpoint payloads | Product behavior does not belong in Foundation. | Owning feature task design. |
