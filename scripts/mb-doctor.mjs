...
function checkFeatureAcceptanceTrace(records) {
  ...
  if (isFile(requirementsAbs)) {
    try {
      requirementsText = fs.readFileSync(requirementsAbs, 'utf8');
    } catch {
      requirementsText = '';
    }
  }
...
function hasAcceptanceExecutionEvidence(taskId, acId, notApplicable) {
  const progressRel = normalizeRel(path.join('.protocols', taskId, 'progress.md'));
  const verificationRel = normalizeRel(path.join('.protocols', taskId, 'verification.md'));
